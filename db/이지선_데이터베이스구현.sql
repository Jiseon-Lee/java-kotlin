...
select left(m_date, 7), count(mi_id)		-- *넣어도 되는데 primary key 넣는게 간단
from t_member
group by year(m_date), month(m_date);		-- 년도별로 묶은 다음 월별로 묶는다. 이게 정석... 이런식으로 묶을 경우 year(m_date), month(m_date)마다 조건을 줄 수 있음

-- 가장 최근 가입한 10명을 추출하는 쿼리 작성
select * from t_member order by m_idx desc limit 10;	-- m_idx가 보조인덱스이기 때문에 조금 더 효율적 
select * from t_member order by m_date desc limit 10;
